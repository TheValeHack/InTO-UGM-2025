...
export default function BubbleButton({children, className, onClick, scale  = 1, color = 'default' }){
    return <div 
                onClick={onClick}
                className={cn(
                    "w-min text-center font-superbubble py-3 bg-center bg-no-repeat bg-contain flex items-center justify-center drop-shadow-[0_4px_5px_rgba(0,0,0,0.25)] relative cursor-pointer transition-all hover:translate-y-1",
                    ...
                    className)}
            >
                <div className=" button-text-shadow bg-[#FAFAFA] text-transparent z-10">
                {children}
                </div>
